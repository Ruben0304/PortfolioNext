    ...
    <div className={cn("relative block md:hidden", className)}>
      <AnimatePresence>
        {open && (
          ...
          >
            {items.map((item, idx) => {
              const isThemeToggler = item.href === '#';
              const content = (
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-muted dark:bg-neutral-900">
      ...
      >
        <IconLayoutNavbarCollapse className="h-5 w-5 text-muted-foreground dark:text-neutral-400" />
      </button>
    </div>
  );
};
